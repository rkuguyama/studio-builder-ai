...
import { startHttpBridgeServer } from "./ipc/bridge/http_bridge_server";
...
export async function onReady() {
  ...
  // In headless mode, always start the bridge regardless of DYAD_ENABLE_WEB_BRIDGE
  if (IS_HEADLESS || process.env.DYAD_ENABLE_WEB_BRIDGE === "1") {
    bridgeServer = startHttpBridgeServer({
      port: Number(process.env.DYAD_WEB_BRIDGE_PORT ?? "4310"),
      host: process.env.DYAD_WEB_BRIDGE_HOST ?? "0.0.0.0",
      authToken: process.env.DYAD_WEB_BRIDGE_TOKEN,
      rateLimitPerMinute: Number(process.env.DYAD_RATE_LIMIT_PER_MINUTE ?? "200"),
    });
  }
  ...
  if (!IS_HEADLESS) {
    ...
    logger.info("Auto-update disabled in headless mode");
  }
}
...
function showErrorBoxSafe(title: string, message: string): void {
  if (IS_HEADLESS) {
...
async function handleDeepLinkReturn(url: string) {
  ...
  if (parsed.hostname === "dyad-pro-return") {
    ...
    if (!apiKey) {
      showErrorBoxSafe("Invalid URL", "Expected key");
      return;
    }
    handleDyadProReturn({ apiKey });
    mainWindow?.webContents.send("deep-link-received", {
      type: parsed.hostname,
    });
    return;
  ...
  if (parsed.hostname === "add-mcp-server") {
    ...
    if (!name || !config) {
      showErrorBoxSafe("Invalid URL", "Expected name and config");
      return;
    }
  ...
  if (parsed.hostname === "add-prompt") {
    ...
    if (!data) {
      showErrorBoxSafe("Invalid URL", "Expected data parameter");
      return;
    }
  ...
  showErrorBoxSafe("Invalid deep link URL", url);
}

// In headless mode, don't quit when there are no windows (there never are any).
